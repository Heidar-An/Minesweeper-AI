...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighboursRemove = set()
        for neighbour in neighbours:
            if neighbour in self.mines:
                neighboursRemove.add(neighbour)
                count -= 1
            if neighbour in self.safes:
                neighboursRemove.add(neighbour)
        neighbours -= neighboursRemove
        ...
        for knowledge in self.knowledge:
            if knowledge.cells == set():
                remove.append(knowledge)
        for knowledge in remove:
        ...
        for knowledge in self.knowledge:
            if sentence.cells != knowledge.cells:
                if knowledge.cells.issubset(sentence.cells):
                    knowledgeAdd.append(Sentence(knowledge.cells - sentence.cells, knowledge.count - sentence.count))
                if sentence.cells.issubset(knowledge.cells):
                    knowledgeAdd.append(Sentence(sentence.cells - knowledge.cells, sentence.count - knowledge.count))
        self.knowledge += knowledgeAdd
